...
df = df_stock.set_index('Date')[['Close']]
...
df_input = df_train_test['Close']
X = []
y = []
window = 15
for i in range(window, len(df_input)):
    X.append(df_input[i-window:i])
    y.append(df_input[i])
X=np.array(X)
y=np.array(y)
...
closing_price_model.add(tf.keras.layers.LSTM(500, activation='tanh', return_sequences=True, input_shape=(X_train.shape[1], 1)))
...
closing_price_model.add(tf.keras.layers.Dropout(0.2))
closing_price_model.add(tf.keras.layers.Dense(1, activation='linear'))
...
test_results = pd.DataFrame({'Predicted_Close': predicted_values_on_test_data[:, 0]})
...
while j < len(df_past_is_future.index):
    input_buffer = np.reshape(input_buffer, (1, window))
    future_price_element = closing_price_model.predict(input_buffer)
    input_buffer = np.delete(input_buffer, 0)
    input_buffer = np.insert(input_buffer, len(input_buffer), future_price_element[0], axis=0)
    if j == 0:
        y_future = np.array(future_price_element)
    else:
        y_future = np.append(y_future, future_price_element)
    j = j + 1

df_future = pd.DataFrame({'Future_Close': y_future})
# assign proper Date index to predicted values
df_future.index = df_past_is_future.index[0:len(y_future)]
...
df_y_test_predict = pd.DataFrame(predicted_values_on_test_data[:,0])
...
mape_train_score = mean_absolute_percentage_error(y_train, y_train_predict[:,0])
...
# Mean Absolute Error
mae_train_score = mean_absolute_error(y_train, y_train_predict[:,0])
mae_test_score = mean_absolute_error(y_test, predicted_values_on_test_data[:,0])
mae_future_score = mean_absolute_error(df_past_is_future.Close, df_future.Future_Close)

# Mean Squared Error
mse_train_score = mean_squared_error(y_train, y_train_predict[:,0])
mse_test_score = mean_squared_error(y_test, predicted_values_on_test_data[:,0])
mse_future_score = mean_squared_error(df_past_is_future.Close, df_future.Future_Close)

# Root Mean Squared Error
rmse_train_score = np.sqrt(mean_squared_error(y_train, y_train_predict[:,0]))
rmse_test_score = np.sqrt(mean_squared_error(y_test, predicted_values_on_test_data[:,0]))
...
ax.legend()
plt.title('LSTM - Univariate Stock Price Forecasting', pad = 50)
